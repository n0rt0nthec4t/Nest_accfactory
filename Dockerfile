...
# version of node docker we will use. Pegged at node v18.x as bug in docker builds on armv7 when using later versions
ARG NODE_VERSION=18-alpine

# version of our project. pass in via build, formatted as vx.x.x
ARG NEST_ACCFACTORY_VESRION

# first up, we want to build our ffmpeg binary to be included in the final image
FROM node:${NODE_VERSION} as ffmpeg
ARG TARGETARCH
ARG FFMPEG_VERSION
...
COPY --from=ffmpeg /build/ffmpeg-${FFMPEG_VERSION}/ffmpeg ./
...
LABEL org.opencontainers.image.version=${NEST_ACCFACTORY_VESRION}