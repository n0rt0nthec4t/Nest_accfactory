    ...
    #processData(connectionType, deviceUUID) {
        ...
        Object.entries(this.#rawData).filter(([key, value]) => (key.startsWith('device.') === true || (key.startsWith('DEVICE_') === true && PROTOBUF_THERMOSTAT_RESOURCES.includes(value.value?.device_info?.typeName) === true)) && (deviceUUID === '' || deviceUUID === key)).forEach(([object_key, value]) => {
            ...
            if (Object.entries(tempDevice).length !== 0 && typeof devices[tempDevice.serial_number] === 'undefined') {
                ...
                tempDevice.externalCool = (typeof this.config?.devices?.[tempDevice.serial_number]?.externalCool === 'string' ? this.config.devices[tempDevice.serial_number].externalCool : undefined); // Config option for external cooling source
                tempDevice.externalHeat = (typeof this.config?.devices?.[tempDevice.serial_number]?.externalHeat === 'string' ? this.config.devices[tempDevice.serial_number].externalHeat : undefined); // Config option for external heating source
                tempDevice.externalFan = (typeof this.config?.devices?.[tempDevice.serial_number]?.externalFan === 'string' ? this.config.devices[tempDevice.serial_number].externalFan : undefined); // Config option for external fan source
                tempDevice.externalDehumidifier = (typeof this.config?.devices?.[tempDevice.serial_number]?.externalDehumidifier === 'string' ? this.config.devices[tempDevice.serial_number].externalDehumidifier : undefined);// Config option for external dehumidifier source
        ...
        Object.entries(this.#rawData).filter(([key, value]) => (key.startsWith('quartz.') === true || (key.startsWith('DEVICE_') === true && PROTOBUF_CAMERA_DOORBELL_RESOURCES.includes(value.value?.device_info?.typeName) === true)) && (deviceUUID === '' || deviceUUID === key)).forEach(([object_key, value]) => {
            ...
            if (Object.entries(tempDevice).length !== 0 && typeof devices[tempDevice.serial_number] === 'undefined') {
                ...
                tempDevice.doorbellCooldown  = (typeof this.config?.devices?.[tempDevice.serial_number]?.doorbellCooldown === 'number' ? this.config.devices[tempDevice.serial_number].doorbellCooldown : 60);
                tempDevice.motionCooldown = (typeof this.config?.devices?.[tempDevice.serial_number]?.motionCooldown === 'number' ? this.config.devices[tempDevice.serial_number].personCooldown : 120);
                tempDevice.personCooldown = (typeof this.config?.devices?.[tempDevice.serial_number]?.personCooldown === 'number' ? this.config.devices[tempDevice.serial_number].motionCooldown : 60);
